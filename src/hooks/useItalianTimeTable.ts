...
/**
 * IMPORTANTE: Os minutos dos jogos para o Campeonato Italiano SEMPRE correspondem exatamente 
 ...
 */
export const ITALIAN_MINUTE_BLOCKS = [2, 5, 8, 11, 14, 17, 20, 23, 26, 29, 32, 35, 38, 41, 44, 47, 50, 53, 56, 59];
 ...
 * conforme os blocos de minutos fixos da tabela italiana
 * 
 * REGRA CRÍTICA: Os jogos SEMPRE começam exatamente nos minutos específicos definidos em ITALIAN_MINUTE_BLOCKS.
...
function extractTimeInfo(startTime: string): { hour: number; minute: number; isValidMinute: boolean } {
  try {
    ...
    // Verificar explicitamente se o minuto extraído corresponde exatamente a um dos minutos da tabela italiana
    // Isso é CRUCIAL para o correto posicionamento dos jogos na tabela
    const isValidMinute = ITALIAN_MINUTE_BLOCKS.includes(minute);
...
/**
 * Função para ordenar as horas na sequência correta:
 * Primeiro a hora atual do sistema, depois as demais em ordem decrescente
 */
function sortHoursInCorrectOrder(hours: number[]): number[] {
  ...
  return [...hours].sort((a, b) => {
    // A hora atual sempre vem primeiro
    if (a === currentHour) return -1;
    if (b === currentHour) return 1;
    
    // Para as demais horas, simplesmente ordem decrescente
    return b - a;
  });
}
...
/**
 * Hook para converter dados da API em uma tabela organizada por hora e minuto específicos para o Campeonato Italiano
 * 
 ...
 */
export function useItalianTimeTable(
  matches: IMatch[] = [], 
  ...
  return useMemo(() => {
    ...
    if (testMode) {
      if (selectedHours && selectedHours.length > 0) {
        ...
        // Caso contrário, usar a hora atual como padrão para testes
        filteredHours = [new Date().getHours()];
      }
    } else {
      // Obter a hora atual
      const currentHour = new Date().getHours();
    ...
    // Usar blocos de minutos para o Campeonato Italiano
    const minuteBlocks = ITALIAN_MINUTE_BLOCKS;
