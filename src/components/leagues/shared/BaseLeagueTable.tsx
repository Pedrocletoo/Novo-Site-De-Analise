  ...
  const renderCell = (hour: number, minuteStr: string, rowCells: TableCell[]) => {
    ...
    if (finalRenderOptions.marketFilters && shouldShowOdds && finalRenderOptions.activeMarketFilter) {
      ...
      if (activeFilter && activeFilter.active) {
        ...
        if ((activeFilter.type === 'over' && (activeFilter.value === '4.5' || activeFilter.value === '5.5')) || 
            (activeFilter.type === 'under' && activeFilter.value === '4.5')) {
          ...
          oddValue = getOddValue(
            ...
            activeFilter.type, 
            activeFilter.value, 
            finalRenderOptions.isBR4 || false
          );
        }
      }
    }
    ...
    return (
      <DataCell key={`cell-${minuteStr}`} {...cellColor}>
        ...
        {oddValue !== null ? (
          <OddValue>{oddValue.toFixed(2)}</OddValue>
        ) : (
          shouldShowOdds && !isUnsupportedMarket && (
            <OddValue style={{ opacity: 0.5 }}>N/D</OddValue>
          )
        )}
      </DataCell>
    );
